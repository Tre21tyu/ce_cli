...
export class StackManager {
  ...
  public async addWorkOrderToStack(workOrderNumber: string): Promise<string> {
    try {
      ...
      const { services, importTimestamp } = await parseServices(workOrderNumber);
      ...
      const stackableServices = await convertToStackableServices(services, importTimestamp);
...
SUMMARY:
${chalk.cyan('Total Work Orders:')} ${chalk.yellow(totalWorkOrders.toString())}
${chalk.cyan('Total Services:')} ${chalk.yellow(totalServices.toString())}
${chalk.cyan('Total Time:')} ${chalk.yellow(hoursDisplay)}
...
`;
      ...
      stack.forEach((wo, index) => {
        ...
        result += `${index + 1}. Work Order ${chalk.cyan(wo.workOrderNumber)} (${wo.services.length} services, ${workOrderMinutes} minutes)`;
        ...
        if (wo.services.length > 0) {
          wo.services.forEach(service => {
            ...
            if (service.noun_code !== undefined) {
              result += `\n   - (${datePart}) Verb Code: ${service.verb_code}, Noun Code: ${service.noun_code}, Time: ${chalk.yellow((service.serviceTimeCalculated || 0).toString())}`;
            } else {
              result += `\n   - (${datePart}) Verb Code: ${service.verb_code}, Time: ${chalk.yellow((service.serviceTimeCalculated || 0).toString())}`;
            }
          });
        } else {
